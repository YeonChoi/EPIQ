...
namespace InformedProteomics.Backend.MassSpecData
{
    public abstract class LcMsRun: ILcMsRun
    {
        ...
        public Spectrum GetMzLimitedSummedMs1Spectrum(int startScanNum, double minElutionTime, double maxElutionTime, double minMz, double maxMz)
        {
            // Added by Yeon
            var minScanNum = startScanNum;
            var curScanNum = startScanNum;
            while (minScanNum >= MinLcScan)
            {
                curScanNum = GetPrevScanNum(curScanNum, 1);
                var curElutionTime = GetElutionTime(curScanNum);
                if (curElutionTime > minElutionTime) minScanNum = curScanNum;
                else break;
            }

            var maxScanNum = startScanNum;
            curScanNum = startScanNum;
            while (maxScanNum <= MaxLcScan)
            {
                curScanNum = GetNextScanNum(curScanNum, 1);
                var curElutionTime = GetElutionTime(curScanNum);
                if (curElutionTime < maxElutionTime) maxScanNum = curScanNum;
                else break;
            }

            var scanNumList = new List<int>();
            for (var scanNum = minScanNum; scanNum <= maxScanNum; scanNum++)
            {
                if (GetMsLevel(scanNum) != 1) continue;
                scanNumList.Add(scanNum);
            }
            return GetMzLimitedSummedSpectrum(scanNumList, minMz, maxMz, minScanNum);
        }

        public SummedSpectrum GetMzLimitedSummedSpectrum(IList<int> scanNums, double minMz, double maxMz, int repScanNum = 0)
        {
            //Added by Yeon
            ...
            foreach (var scanNum in scanNums)
            {
                ...
                {
                    if (peak.Mz < minMz || peak.Mz > maxMz) continue;
        ...
        public Spectrum GetSummedMs1Spectrum(int startScanNum, double minElutionTime, double maxElutionTime)
        {
            // Added by Yeon
            var minScanNum = startScanNum;
            var curScanNum = startScanNum;
        ...
        /// <param name="tolerance">tolerance</param>
        /// <param name="targetScanNum">target scan number to generate xic</param>
        /// <param name="etsRange">ets range</param>
        /// <returns>XIC around targetScanNum</returns>
        public Xic GetPrecursorExtractedIonChromatogram(double mz, Tolerance tolerance, int targetScanNum, double[] etsRange)
        {
            var tolTh = tolerance.GetToleranceAsTh(mz);
            var minMz = mz - tolTh;
            var maxMz = mz + tolTh;
            if (targetScanNum < 0) return GetPrecursorExtractedIonChromatogram(minMz, maxMz);
            return GetPrecursorExtractedIonChromatogram(minMz, maxMz, targetScanNum, etsRange);
        }
        ...
        /// <param name="maxMz">max m/z</param>
        /// <param name="targetScanNum">target scan number to generate xic</param>
        /// <param name="etsRange">elution time range</param>
        /// <returns>XIC around targetScanNum within Ets range</returns>
        public Xic GetPrecursorExtractedIonChromatogram(double minMz, double maxMz, int targetScanNum, double[] etsRange)
        {
            if (GetMsLevel(targetScanNum) > 1)
                targetScanNum = GetPrecursorScanNum(targetScanNum);
            var xic = GetPrecursorExtractedIonChromatogram(minMz, maxMz);
            return GetTrimmedXic(xic, targetScanNum, etsRange);
        }
        ...
        public Xic GetTrimmedXic(Xic xic, int targetScanNum, double minMz, double maxMz, int tolerance = 3)
        {
            var newXic = new Xic();
            var txic = GetTrimmedXic(xic, targetScanNum, tolerance);
            foreach (var xp in txic)
            {
                if (xp.Mz < minMz || xp.Mz > maxMz) continue;
                newXic.Add(xp);
            }
            return newXic;
        }
        ...
        /// <param name="targetScanNum">target scan number to generate xic</param>
        /// <param name="etsRange">ets range</param>
        /// <returns>Trimmed XIC around targetScanNum within ets range</returns>
        public Xic GetTrimmedXic(Xic xic, int targetScanNum, double[] etsRange)
        {
            ...
            {
                var xicPeak = xic[i];
                if (GetElutionTime(xicPeak.ScanNum) < etsRange[0]) break;
            ...
            {
                var xicPeak = xic[i];
                if (GetElutionTime(xicPeak.ScanNum) > etsRange[1]) break;
                xicSegment.Add(xicPeak);
                ++i;
            }
